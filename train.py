...
parser.add_argument('--pretrained_model', type=str, default='check_point/my_model/pretrain/best_model.pt',
...
parser.add_argument('--alpha_vgg', type=float, default=10)
parser.add_argument('--alpha_gan', type=float, default=2e-1)
parser.add_argument('--alpha_tv', type=float, default=0)
parser.add_argument('--alpha_l1', type=float, default=0)
parser.add_argument('--alpha_l2', type=float, default=2e-2)
...
def main(argv=None):
    ...
    check_point = os.path.join(args.check_point, 'trainv1')
    ...
    for epoch in range(1, args.num_epochs+1):
        ...
        #===============Train======================
            ...
            for i, (inputs, labels) in enumerate(tqdm(train_loader)):
                ...
                l2_loss = l2_loss_fn(sr,hr)*args.alpha_l2
                ...
                total_G_loss = vgg_loss + G_loss + l2_loss
            ...
            tb.add_scalar('L2 Loss', avr_loss[2], epoch)
            tb.add_scalar('D Loss', avr_loss[3], epoch)
            tb.add_scalar('Total G Loss', avr_loss[0:3].sum(), epoch)
            print('Finish train [%d/%d]. VGG: %.2f. G: %.2f. L2: %.2f. Total G: %.2f. D: %.2f'\
                  %(epoch, args.num_epochs, avr_loss[0], avr_loss[1], avr_loss[2], 
                    avr_loss[0:3].sum(), avr_loss[3]))
